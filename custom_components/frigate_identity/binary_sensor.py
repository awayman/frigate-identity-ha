...
from .const import DOMAIN, SERVICE_HEARTBEAT_INTERVAL_SECONDS
...
async def async_setup_entry(
    ...
    service_status = FrigateIdentityServiceStatusSensor(registry)
    ...
    _on_persons_changed()
    async_add_entities([service_status])
...
class FrigateIdentityServiceStatusSensor(BinarySensorEntity):
    """Binary sensor tracking whether the identity service heartbeat is fresh."""
    ...
    def __init__(self, registry: PersonRegistry) -> None:
        """Initialise the sensor."""
        self._registry = registry
        self._attr_name = "Identity Service Status"
        self._attr_unique_id = "frigate_identity_service_status"
        self._attr_is_on = False
        self._attr_extra_state_attributes: dict[str, Any] = {}
        self._unsub_listener: Any = None
    ...
    async def async_added_to_hass(self) -> None:
        """Start tracking heartbeat state."""

        @callback
        def _refresh(*_args: Any) -> None:
            self._update_from_health()

        self._unsub_listener = self._registry.register_listener(_refresh)
        self._unsub_timer = async_track_time_interval(
            self.hass,
            _refresh,
            timedelta(seconds=SERVICE_HEARTBEAT_INTERVAL_SECONDS),
        )
        self._update_from_health()

    @callback
    def _update_from_health(self) -> None:
        """Update state from registry health data."""
        health = self._registry.get_service_health()
        self._attr_is_on = health["is_connected"]
        self._attr_extra_state_attributes = {
            "status": health["status"],
            "last_heartbeat_age_seconds": health["last_heartbeat_age_seconds"],
            "last_heartbeat_timestamp": health["last_heartbeat_timestamp"],
        }
        self.async_write_ha_state()
